...
import { name } from "@/store/test/test1"
import { useRecoilValue ,useRecoilState} from "recoil"
import { doctorVisitsState } from "@/store/doctors"
...
export default function DasBoardWrapperComp() {
//   const { data: session } = useSession()
//   const [visits, setVisits] = useState<DoctorVisit[]>([]);
//   const thename = useRecoilValue(name);
...
const { data: session } = useSession();
  ...
  // Move Recoil value after session and state initialization
  const thename = useRecoilValue(name);
  ...
  useEffect(() => {
    const getVisits = async () => {
      try {
        ...
        console.log(" doctor visits in dashboard",data);
        setVisits(data);
      } catch (error) {
        console.error('Error fetching visits:', error);
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="px-4 py-8">
        <div className="mb-8">
          <h2 className="text-3xl font-bold text-gray-800 mb-2">{thename}'s Clinic Dashboard 🏥</h2>
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6">
          <h3 className="text-xl font-semibold text-gray-800 mb-4">Active Visits</h3>
            {visits.length === 0 ? (
